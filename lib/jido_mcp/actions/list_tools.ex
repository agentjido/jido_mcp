defmodule Jido.MCP.Actions.ListTools do
  @moduledoc "List tools for a configured MCP endpoint."

  use Jido.Action,
    name: "mcp_tools_list",
    description: "List tools from an MCP endpoint",
    schema:
      Zoi.object(%{
  ...
  def run(params, context) do
    with {:ok, endpoint_id} <- Helpers.resolve_endpoint_id(params, context) do
      ...
      Jido.MCP.list_tools(endpoint_id, opts)
    end
  end
