defmodule Jido.MCP.Actions.ListPrompts do
  @moduledoc "List prompts for a configured MCP endpoint."

  use Jido.Action,
    name: "mcp_prompts_list",
    description: "List prompts from an MCP endpoint",
    schema:
      Zoi.object(%{
  ...
  def run(params, context) do
    with {:ok, endpoint_id} <- Helpers.resolve_endpoint_id(params, context) do
      ...
      Jido.MCP.list_prompts(endpoint_id, opts)
    end
  end
